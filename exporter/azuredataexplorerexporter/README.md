...
| Code coverage | [![codecov](https://codecov.io/github/open-telemetry/opentelemetry-collector-contrib/graph/main/badge.svg?component=exporter_azuredataexplorer)](https://app.codecov.io/gh/open-telemetry/opentelemetry-collector-contrib/tree/main/?components%5B0%5D=exporter_azuredataexplorer&displayType=list) |
...
| Emeritus      | [@asaharn](https://www.github.com/asaharn) |
...
This exporter sends metrics, logs, trace and profile data to
...
- `profiles_table_name` (default = OTELProfiles): The target table in the database `db_name` that stores exported profiles data.
...
- `profiles_table_json_mapping` (optional, no default): The table mapping name to be used for the table `db_name`.`profiles_table_name`
...
exporters:
  azuredataexplorer:
    ...
    # Profiles table
    profiles_table_name: "OTELProfiles"
    ...
    profiles_table_json_mapping: "otelprofiles_mapping"
...
| ADX Table column              | Description / OpenTelemetry attribute                 |
| ----------------------------- |------------------------------------------------------ |
| Timestamp                     | The timestamp of the profile or individual sample     |
| Duration                      | The duration of the profile collection period         |
| ProfileID                     | A globally unique identifier for the profile          |
| SampleType                    | The type of value measured (e.g. "cpu", "allocated_objects") |
| SampleUnit                    | The unit of the measured value (e.g. "nanoseconds", "bytes") |
| SampleValue                   | The measured value for this sample                    |
| StackTrace                    | Resolved stack trace as an array of frames, each with Function, SystemName, File, Line, Column, Mapping, Address. Leaf frame first. |
| StackDepth                    | Number of frames in the stack trace                   |
| TraceID                       | Trace ID from a linked span, if present               |
| SpanID                        | Span ID from a linked span, if present                |
| SampleAttributes              | Attributes specific to this sample                    |
| ResourceAttributes            | The resource attributes JSON map as specified in open telemetry [resource semantics](https://opentelemetry.io/docs/reference/specification/resource/semantic_conventions/) |
| ScopeAttributes               | [Instrumentation scope](https://opentelemetry.io/docs/reference/specification/common/#attribute) name and version |
...
.create-merge table <Profiles-Table-Name> (Timestamp:datetime, Duration:string, ProfileID:string, SampleType:string, SampleUnit:string, SampleValue:long, StackTrace:dynamic, StackDepth:int, TraceID:string, SpanID:string, SampleAttributes:dynamic, ResourceAttributes:dynamic, ScopeAttributes:dynamic)
